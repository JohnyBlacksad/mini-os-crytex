...
| Agent chain execution | PARTIAL | Real Ollama smoke proves architect/coder/qa/security/critic chain; Tauri tests verify upstream artifacts are attached | Needs stronger artifact contract validation, not just status/result JSON | E2E verifies each task consumes previous artifact/result and emits concrete artifact matching schema |
| Critic rejection/remediation | PARTIAL | Smoke has `critic_rejected` and `remediation_plan_created`; tests assert feedback/failure type/target task in several paths | Need structured rejection schema enforced as a required contract for every critic backend | Tests assert reason, blocking issues, target task, remediation task linkage for real and mock paths |
...
| AST/chunking/code graph | PARTIAL | `crytex-doc` tree-sitter chunking, graph builder, impact tests passed; kernel builds `CodeGraph`; project indexer now uses graph-aware code chunking and persists `symbol_id`/`related_symbols` payload metadata | RAG prompt/diagnostics still need to expose graph scope clearly | Integration: index mixed project, assert graph metadata reaches selected RAG prompt and diagnostics |
| RAG/Qdrant Edge | PARTIAL | Indexer, Edge vector store, context assembler, hybrid dense+sparse tests passed; real smoke proves marker reaches model; reranker ordering unit proves final context order can be changed by rerank | Need PDF/doc proof and real kernel/Tauri diagnostics for rerank decisions | Integration: index mixed project, query, rerank, assert top result, prompt inclusion, and before/after rank diagnostics |
| Reranker | PARTIAL | Trait and ONNX implementation exist; ONNX config tests exist; `ContextAssembler::with_reranker` is tested; kernel wires `create_reranker` into context assembly | Tauri app path still uses mock context components; diagnostics do not yet show before/after ranks | Integration proving reranker changes ordering in the agent execution path and diagnostics |
...
| Agent implementations | PARTIAL | Role agents, prompts, JSON parsing, tooling tests exist | Mostly mock-backed; loose JSON contracts | Typed agent result contracts, malformed-output repair tests, real local smoke per role |
