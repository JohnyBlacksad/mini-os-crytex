...
#[cfg(feature = "cuda")]
fn unquantized_qmatmul_indexed_moe_forward(
    qmatmul: &QMatMul,
    x: &Tensor,
    indices: &Tensor,
) -> Result<Tensor> {
    let w = match qmatmul {
        QMatMul::Tensor(w) | QMatMul::TensorF16(w) => w,
        QMatMul::QTensor(_) => candle_core::bail!("QTensor should use the CUDA indexed MoE kernel"),
    };

    let (num_tokens, input_experts_per_tok, hidden_dim) = x.dims3()?;
    let (_, experts_per_tok) = indices.dims2()?;
    let flat_indices = indices.reshape((num_tokens * experts_per_tok,))?;
    let selected_w = w.index_select(&flat_indices, 0)?;
    let out_features = selected_w.dim(1)?;

    let expanded_x = match input_experts_per_tok {
        1 => x.broadcast_as((num_tokens, experts_per_tok, hidden_dim))?,
        n if n == experts_per_tok => x.clone(),
        n => candle_core::bail!(
            "indexed MoE input top-k mismatch: input has {n}, indices have {experts_per_tok}"
        ),
    };

    expanded_x
        .reshape((num_tokens * experts_per_tok, hidden_dim))?
        .unsqueeze(1)?
        .matmul(&selected_w.transpose(1, 2)?)?
        .squeeze(1)?
        .reshape((num_tokens, experts_per_tok, out_features))
}
...
mod tests {
    ...
    #[test]
    fn should_gather_unquantized_tensor_weights_without_requiring_qtensor() -> Result<()> {
        ...
        let input = Tensor::from_vec(vec![3.0f32, 5.0], (1, 1, 2), &device)?;
        let indices = Tensor::from_vec(vec![1u32], (1, 1), &device)?;
        let layer = GgufMatMul {
            w: QMatMul::Tensor(weights),
            b: None,
        };
        ...
        assert_eq!(output.to_vec3::<f32>()?, vec![vec![vec![6.0, 10.0]]]);
        Ok(())
    }
}
...
impl QuantMethod for GgufMatMul {
    ...
    fn gather_forward(&self, x: &Tensor, indices: &Tensor) -> Result<Tensor> {
        ...
        let res = match &self.w {
            QMatMul::QTensor(_) => cuda::qmatmul_indexed_moe_forward(&self.w, x, indices)?,
            QMatMul::Tensor(_) | QMatMul::TensorF16(_) => {
                unquantized_qmatmul_indexed_moe_forward(&self.w, x, indices)?
            }
        };
