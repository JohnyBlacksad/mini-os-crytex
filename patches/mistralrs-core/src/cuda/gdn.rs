...
#[cfg(all(feature = "cuda", not(mistralrs_skip_gdn_cuda)))]
...
#[cfg(any(not(feature = "cuda"), mistralrs_skip_gdn_cuda))]
#[allow(unused)]
pub fn gated_delta_rule_recurrence_cuda(
    _q: &Tensor,
    _k: &Tensor,
    ...
    candle_core::bail!("gated_delta_rule_recurrence_cuda requires the CUDA GDN kernel")
}
...
pub fn chunked_gated_delta_rule_recurrence_cuda(
    ...
    candle_core::bail!("chunked_gated_delta_rule_recurrence_cuda requires the CUDA GDN kernel")
}
...
pub fn causal_conv1d_cuda(
    ...
    candle_core::bail!("causal_conv1d_cuda requires the CUDA GDN kernel")
}
...
pub fn fused_gdn_gating_cuda(
    ...
    candle_core::bail!("fused_gdn_gating_cuda requires the CUDA GDN kernel")
}
...
mod tests {
    ...
    #[cfg(all(feature = "cuda", not(mistralrs_skip_gdn_cuda)))]
    #[test]
    fn cuda_chunked_gdn_matches_cpu_reference() -> Result<()> {
        ...
        let q_host = patterned_values(bh * seq_len * k_dim, 0.013, -0.031);
        let k_host = patterned_values(bh * seq_len * k_dim, -0.017, 0.047);
        let v_host = patterned_values(bh * seq_len * v_dim, 0.019, -0.023);
        let g_host = vec![-0.02; bh * seq_len];
        let beta_host = vec![0.37; bh * seq_len];
        let state_host = patterned_values(bh * k_dim * v_dim, 0.007, 0.011);

        let (expected_output, expected_state) = cpu_gdn_reference(
            &q_host,
            &k_host,
            &v_host,
            &g_host,
        ...
        let q = Tensor::from_vec(q_host, (bh, seq_len, k_dim), &device)?;
        let k = Tensor::from_vec(k_host, (bh, seq_len, k_dim), &device)?;
        let v = Tensor::from_vec(v_host, (bh, seq_len, v_dim), &device)?;
        let g = Tensor::from_vec(g_host, (bh, seq_len), &device)?;
        let beta = Tensor::from_vec(beta_host, (bh, seq_len), &device)?;
        let mut state = Tensor::from_vec(state_host, (bh, k_dim, v_dim), &device)?;

        let output =
            chunked_gated_delta_rule_recurrence_cuda(&q, &k, &v, &g, &beta, &mut state)?;

        assert_close(
            &output.to_dtype(DType::F32)?.flatten_all()?.to_vec1::<f32>()?,
            &expected_output,
            2e-4,
        );
        assert_close(
    ...
    #[cfg(all(feature = "cuda", not(mistralrs_skip_gdn_cuda)))]
    fn patterned_values(len: usize, scale: f32, offset: f32) -> Vec<f32> {
        (0..len)
            .map(|idx| {
    ...
    fn cpu_gdn_reference(
        ...
        let mut state = state_initial.to_vec();
        let mut output = vec![0.0; bh * seq_len * v_dim];

        for bh_idx in 0..bh {
            let q_base = bh_idx * seq_len * k_dim;
            let k_base = bh_idx * seq_len * k_dim;
            let v_base = bh_idx * seq_len * v_dim;
            let g_base = bh_idx * seq_len;
            let state_base = bh_idx * k_dim * v_dim;

            for t in 0..seq_len {
                let decay = g[g_base + t].exp();
                let beta_t = beta[g_base + t];

                for v_idx in 0..v_dim {
                    let v_t = v[v_base + t * v_dim + v_idx];
                    let mut kv_mem = 0.0;
                    for j in 0..k_dim {
                        let state_idx = state_base + j * v_dim + v_idx;
                        state[state_idx] *= decay;
                        kv_mem += state[state_idx] * k[k_base + t * k_dim + j];
                    }
                    ...
                    for j in 0..k_dim {
                        let state_idx = state_base + j * v_dim + v_idx;
                        state[state_idx] += k[k_base + t * k_dim + j] * delta;
                        y_t += state[state_idx] * q[q_base + t * k_dim + j];
                    }

                    output[v_base + t * v_dim + v_idx] = y_t;
                }
            }
        }
    ...
    fn assert_close(actual: &[f32], expected: &[f32], tolerance: f32) {
        ...
        for (idx, (actual, expected)) in actual.iter().zip(expected).enumerate() {
            ...
            assert!(
                diff <= tolerance,
                "value mismatch at {idx}: actual={actual}, expected={expected}, diff={diff}, tolerance={tolerance}"
            );
        }
    }
}
