...
/// A portable WebAssembly value used when invoking guest exports.
#[derive(Clone, Debug)]
pub enum WasmValue {
    I32(i32),
    I64(i64),
...
impl SandboxError {
    fn classify_trap(err: &wasmtime::Error) -> Self {
        if err.is::<I32Exit>() {
            ...
            SandboxError::Engine(anyhow::Error::msg(err.to_string()))
...
/// Trait abstracting a secure WASM sandbox.
...
/// Wasmtime-based implementation of [`Sandbox`].
...
impl Sandbox for WasmtimeSandbox {
    async fn run(
        ...
        let instance = linker
            ...
            .await
            .map_err(|e| SandboxError::classify_trap(&e))?;
        ...
        let exit_code = match func.call_async(&mut store, ()).await {
            ...
            Err(err) => {
                if let Some(exit) = err.downcast_ref::<I32Exit>() {
                    ...
                    return Err(SandboxError::classify_trap(&err));
                }
            }
        };
...
mod tests {
    ...
    async fn should_return_error_when_start_export_missing() {
        ...
        let sandbox = WasmtimeSandbox::new();
        let err = sandbox
            .run(&wasm, &SandboxConfig::default())
            .await
            .unwrap_err();

        assert!(
            matches!(err, SandboxError::Engine(_)),
