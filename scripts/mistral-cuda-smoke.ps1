...
function Invoke-LoggedProcess {
    ...
    if ($null -eq $exitCode) {
        $hasTestOk =
            (Select-String -LiteralPath $StdoutPath -SimpleMatch "test result: ok" -Quiet) -or
            (Select-String -LiteralPath $StderrPath -SimpleMatch "test result: ok" -Quiet)
        $hasError =
            (Select-String -LiteralPath $StdoutPath -Pattern "error:|test result: FAILED|panicked|fatal error" -Quiet) -or
            (Select-String -LiteralPath $StderrPath -Pattern "error:|test result: FAILED|panicked|fatal error" -Quiet)
        $exitCode = if ($hasTestOk -and -not $hasError) { 0 } else { 1 }
    }
...
$testName = "real_hf_tiny_gguf_downloaded_model_generates_with_mistralrs"
$runEnvName = "CRYTEX_RUN_SLOW_MISTRAL_SMOKE"
$logPrefix = "mistral-cuda-smoke"
if ($Gdn) {
    $testName = "real_hf_tiny_qwen3_next_gdn_generates_with_mistralrs"
    $runEnvName = "CRYTEX_RUN_SLOW_MISTRAL_GDN_SMOKE"
    $logPrefix = "mistral-cuda-gdn-smoke"
}

if ($SkipGdnCuda) {
    $env:MISTRALRS_SKIP_GDN_CUDA = "1"
}
Set-Item -Path "env:$runEnvName" -Value "1"
$env:CRYTEX_MISTRAL_SMOKE_DEVICE = $Device

if ($GpuLayers.Trim().Length -gt 0) {
    $env:CRYTEX_MISTRAL_SMOKE_GPU_LAYERS = $GpuLayers
}
...
$stdoutPath = Join-Path $LogDir "$logPrefix-$timestamp.out.log"
$stderrPath = Join-Path $LogDir "$logPrefix-$timestamp.err.log"

Write-Host "CUDA smoke preflight:"
...
Write-Host "  CRYTEX_MISTRAL_SMOKE_DEVICE=$env:CRYTEX_MISTRAL_SMOKE_DEVICE"
Write-Host "  $runEnvName=$((Get-Item -Path "env:$runEnvName").Value)"
...
if ($env:CRYTEX_MISTRAL_SMOKE_GPU_LAYERS) {
    Write-Host "  CRYTEX_MISTRAL_SMOKE_GPU_LAYERS=$env:CRYTEX_MISTRAL_SMOKE_GPU_LAYERS"
}

try {
    $arguments = @(
        "test",
        "-p", "crytex-inference-mistral",
        "--features", "cuda",
        $testName,
        "--",
        "--ignored",
    ...
    Write-Host "CUDA smoke finished: exit=$($result.ExitCode), timed_out=$($result.TimedOut), duration_seconds=$($result.DurationSeconds)"
    if ($result.ExitCode -ne 0) {
        throw "CUDA smoke failed. stdout=$stdoutPath stderr=$stderrPath"
    }
}
finally {
    if (-not $SkipClean) {
